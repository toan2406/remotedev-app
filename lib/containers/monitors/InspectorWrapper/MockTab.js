  ...
  function MockTab(props) {
    ...
    _this.handleSubmit = function () {
      try {
        ...
        var isValidMockData = (0, _validateMockData2.default)(mockData);

        if (!isValidMockData) throw new Error();

        _this.props.submitMock(mockData);
        _this.props.liftedDispatch({ type: 'APPLY_MOCK', mockData: mockData });
        _this.setState({ error: '', isDirty: false });
      } catch (err) {
        _this.setState({ error: 'Invalid mock data shape' });
      }
    };

    _this.handleRemove = function () {
      return _this.setState({ value: _json2.default.stringify(_mock.DEFAULT_MOCK_DATA) }, _this.handleSubmit);
    };
...
function mapStateToProps(state) {
  return {
    mockData: state.mock ? state.mock.data : _mock.DEFAULT_MOCK_DATA
  };
}
